...
async function handlePurchaseEvent(
  ...
  const existing = await db.execute(
    sql`SELECT id FROM as.licenses WHERE license_key = ${licenseKey} LIMIT 1`
  );
...
async function handleUpgradeEvent(licenseKey: string, tier: any, prevLicenseKey?: string) {
  ...
  let oldLicenseResult = await db.execute(
    sql`SELECT organization_id FROM as.licenses WHERE license_key = ${prevLicenseKey} LIMIT 1`
  );
  ...
  if (!Array.isArray(oldLicenseResult) || oldLicenseResult.length === 0) {
    ...
    oldLicenseResult = await db.execute(
      sql`SELECT organization_id FROM as.licenses WHERE organization_id IS NOT NULL ORDER BY updated_at DESC LIMIT 1`
    );
