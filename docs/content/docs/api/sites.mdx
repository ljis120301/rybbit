...
<Tabs items={['cURL', 'JavaScript', 'Python', 'PHP', 'Ruby', 'Go', 'Rust', 'Java', '.NET']}>
  <Tab value="cURL">
```bash title="Request"
curl -X POST "https://api.rybbit.io/api/sites" \
  -H "Authorization: Bearer your_api_key_here" \
  -H "Content-Type: application/json" \
  -d '{"domain": "example.com", "name": "My Website", "organizationId": "org_123", "blockBots": true}'
```
  </Tab>
  <Tab value="JavaScript">
```javascript title="Request"
const response = await fetch(
  'https://api.rybbit.io/api/sites',
  {
    method: 'POST',
    headers: {
...
```php title="Request"
$ch = curl_init();
curl_setopt($ch, CURLOPT_URL, 'https://api.rybbit.io/api/sites');
...
uri = URI('https://api.rybbit.io/api/sites')
...
req, _ := http.NewRequest("POST", "https://api.rybbit.io/api/sites", body)
...
let res = client
    .post("https://api.rybbit.io/api/sites")
...
String json = "{\"domain\": \"example.com\", \"name\": \"My Website\", \"organizationId\": \"org_123\", \"blockBots\": true}";
HttpRequest request = HttpRequest.newBuilder()
    .uri(URI.create("https://api.rybbit.io/api/sites"))
    .header("Authorization", "Bearer your_api_key_here")
...
var content = new StringContent(
    "{\"domain\": \"example.com\", \"name\": \"My Website\", \"organizationId\": \"org_123\", \"blockBots\": true}",
    Encoding.UTF8,
    "application/json"
);

var response = await client.PostAsync("https://api.rybbit.io/api/sites", content);
...
Permanently deletes a site and all its associated data. This action cannot be undone.
...
Updates site configuration settings. All fields are optional - only include the fields you want to update.
...
Generates or revokes a private link key for sharing analytics without authentication.
